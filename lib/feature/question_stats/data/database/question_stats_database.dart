...
class QuestionStatsDatabase extends DatabaseAccessor<AppDatabase>
  ...
  Future<QuestionStatsDatabaseDto?> getByQuestionKey(String key) =>
      (select(questionStats)..where((t) => t.questionKey.equals(key))).getSingleOrNull();

  Future<List<QuestionStatsDatabaseDto>> getLeastRemembered(int limit) => (select(questionStats)
        ..orderBy([
  ...
  Future<void> upsertStat({
    ...
    if (existing != null) {
      await (update(questionStats)..where((t) => t.questionKey.equals(questionKey))).write(
  ...
  Future<void> upsertStats(
    List<({String questionKey, String frontText, String backText, bool isCorrect})> results,
  ) async {
